...
model = ExponentialSmoothing(train['WMT'])
...
train_mae = mean_absolute_error(train['WMT'], train['triple_exp_smoothing_forecast'])
train_mse = mean_squared_error(train['WMT'], train['triple_exp_smoothing_forecast'])
train_mape = mean_absolute_percentage_error(train['WMT'], train['triple_exp_smoothing_forecast'])

# Calculate errors for test set
test_mae = mean_absolute_error(test['WMT'], test['triple_exp_smoothing_forecast'])
test_mse = mean_squared_error(test['WMT'], test['triple_exp_smoothing_forecast'])
test_mape = mean_absolute_percentage_error(test['WMT'], test['triple_exp_smoothing_forecast'])
...
plt.plot(train['Date'], train['triple_exp_smoothing_forecast'], label='Training triple Exp Smoothing Forecast', linestyle='--')
plt.plot(test['Date'], test['WMT'], label='Test Data')
plt.plot(test['Date'], test['triple_exp_smoothing_forecast'], label='Test triple Exp Smoothing Forecast', linestyle='--')
plt.legend()
plt.xlabel('Date')
plt.ylabel('Value')
